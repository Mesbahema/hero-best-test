...
export async function GET(req: Request) {
    ...
    try {
        ...
        if (axios.isAxiosError(error)) {
            console.error("Axios error fetching coins:", error.message);
        } else if (error instanceof Error) {
            console.error("Unexpected error fetching coins:", error.message);
        } else {
            console.error("Unknown error fetching coins:", error);
        }
    }
}
